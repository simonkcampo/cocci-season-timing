...
1. Loading in data
```{r}
msm.data.census <- read.csv('Data/msm.data.census2023.csv') %>% arrange(GEOID, first_day)

msm.data.county <- read.csv('Data/msm.data.county2023.csv')%>% arrange(county1, first_day)
```
...
```{r}
ppt.tmean.model <- read_rds('ppt.tmean.two.state.2023.rds')
...
png('Plots/census_srm_HRs.png', width = 8, height = 6, units = 'in',res = 500)
cowplot::plot_grid(
  p1)
dev.off()

...
1. Loading in data
```{r}
msm.data.census <- read.csv('~/Desktop/thesis_project/msm.data.census2023.csv') %>% arrange(GEOID, first_day)

msm.data.county <- read.csv('~/Desktop/thesis_project/msm.data.county2023.csv')%>% arrange(county1, first_day)
```



Creating distributed lags
Lags to include
- Median time from symptoms to diagnosis = 55 days (IQR 22 - 143) (Tsang et al.)
- Incubation period = 1-4 weeks (Galgiani)
- Median time from exposure to diagnosis = 2 weeks + ~8 weeks = 10 weeks
Thus, including lags 6-14 weeks

Creating cross-bases
```{r}
library(dlnm)
#county-level
...
Adding crossbasis variables to dataframes
```{r}
#county-level
...
tmean.onset <- crosspred(tmean.county.cb, coef = county.ppt.dlm$estimates[c(9,11,13)],
                   ...
                   model.link = 'log')
# tmean.peak <- crosspred(tmean.county.cb, coef = county.tmean.dlm$estimates[c(5,8,11,14)],
#                    vcov = county.tmean.dlm$covmat[c(5,8,11,14),c(5,8,11,14)],
#                    model.link = 'log')
...
#results.plot %>% dplyr::mutate(CI = paste0(round(HR,2),' ','(',round(lower,2),'-',round(upper,2),')')) %>% write.csv('~/Desktop/results.csv')
...
png('~/Desktop/thesis_project/county_mcm_HRs.png', width = 8, height = 6, units = 'in',res = 500)
cowplot::plot_grid(
  p1)
dev.off()
...
```{r}
ppt.tmean.model <- read_rds('~/Desktop/thesis_project/ppt.tmean.two.state.2023.rds')
...
png('~/Desktop/thesis_project/census_srm_HRs.png', width = 8, height = 6, units = 'in',res = 500)
cowplot::plot_grid(
  p1)
dev.off()
